...
`scripts/canvas-snapshot` performs read-only Canvas CLI calls and normalizes the
results. `scripts/run-canvas-notion-sync` compares the result with the last
...
Every diff-triggered run also appends a dated entry to the Notion Changelog page
(`NOTION_CHANGELOG_PAGE_ID`). The automation never deletes or archives Notion content. Changes that
